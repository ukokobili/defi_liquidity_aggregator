...
def get_utc_from_unix_time(
    ...
    if unix_ts:
        unix_ts_seconds = int(unix_ts) / second  # Convert milliseconds to seconds if needed
        return datetime.datetime.fromtimestamp(unix_ts_seconds, datetime.timezone.utc)  # Create timezone-aware UTC datetime
    else:
        return None
...
def transform_to_add_datetime(token_data) -> pd.DataFrame:
    columns = ['token_name', 'usd', 'usd_market_cap', 'usd_24h_vol', 'usd_24h_change', 'last_updated_at']
